...
#define PREFETCH_SIZE_B 2048
#define PREFETCH_LINES 32  // (PREFETCH_SIZE_B / 64)
#define WPL 4              // Words per cache line.
#define WPS 8              // Words per state (128 bytes).
#define PREFETCH_SHIFT 128 // (PREFETCH_LINES * WPL)
...
static void explode_scratchpad(const __m128i *state, __m128i *ls,
  ...
  for (i = 0; i < memory - PREFETCH_SIZE_B; i += 128) {
    PREFETCH_W(ls + PREFETCH_SHIFT);
    PREFETCH_W(ls + PREFETCH_SHIFT + WPL);
#ifdef __VAES__
    aes_batch(key, x);
#else
    aes_batch(key, x);
...
static void implode_scratchpad(const __m128i *ls, __m128i *state,
  ...
  for (i = 0; i < memory - PREFETCH_SIZE_B; i += 128) {
    PREFETCH_R(ls + PREFETCH_SHIFT);
    PREFETCH_R(ls + PREFETCH_SHIFT + WPL);
#ifdef __VAES__
